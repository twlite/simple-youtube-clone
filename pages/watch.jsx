...
export default class Homepage extends Component {
    ...
    componentDidMount() {
        ...
        if (!this.state.video) fetch(`${window.location.origin}/api/video?id=${q}`)
            .then(res => res.json())
            .then(data => {
                    ...
                    <div className="p-2 ml-10">
                        ...
                        <div className="info border-b-2 border-gray-500 pb-3 flex relative">
                            <div>
                                ...
                                <span className="text-base font-medium opacity-70">{abbrev(this.state.video.views)} views • {this.state.video.uploadedAt}</span>
                            </div>
                            <div className="absolute right-0 bottom-0">
                                <div className="flex text-gray-500 space-x-5 border-gray-600 border-b-2">
                                    <span className="cursor-pointer font-medium" title={this.state.video.ratings.likes.toLocaleString()}><FontAwesomeIcon icon={faThumbsUp} className="h-8 w-8" /> {abbrev(this.state.video.ratings.likes)}</span>
                                    <span className="cursor-pointer font-medium" title={this.state.video.ratings.dislikes.toLocaleString()}><FontAwesomeIcon icon={faThumbsDown} className="h-8 w-8" /> {abbrev(this.state.video.ratings.dislikes)}</span>
                                </div>
                            </div>
                        </div>
